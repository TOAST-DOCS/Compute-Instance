...
アベイラビリティーゾーンの詳細については[インスタンス概要のアベイラビリティーゾーン](./overview/#availability-zone)を参照してください。
...
Floating IP管理は、インスタンス > 管理ページまたはインスタンス > Floating IPページで行えます。Floating IPの詳細は、[VPCコンソール使用ガイド](/Network/VPC/ja/console-guide/)を参照してください。
...
ブロックストレージ管理は、インスタンス > ブロックストレージページでもできます。ブロックストレージの詳細は[ブロックストレージガイド](/Storage/Block%20Storage/ja/overview/)を参照してください。
...
## 付録4. ハイパーバイザのメンテナンスのためのインスタンス再起動ガイド
TOASTは周期的にハイパーバイザのソフトウェアをアップデートして、基本インフラサービスのセキュリティと安定性を向上させています。
メンテナンス対象のハイパーバイザで起動中のインスタンスは再起動を行い、メンテナンスが完了したハイパーバイザに移動する必要があります。
...
![イメージ4](http://static.toastoven.net/prod_instance/instance_p_migration_jp_4.png)
