...
| server.block_device_mapping_v2.volume_type | Body | Enum    | - | 생성되는 블록 스토리지의 타입<br>`사용자 가이드 > Storage > Block Storage > API v2 가이드` 의 **블록 스토리지 타입 목록 보기** 응답의 `name` 참고 |
...
| server.block_device_mapping_v2.volume_size | Body | Integer | - | 생성할 블록 스토리지 크기<br>`GB` 단위<br>루트 블록 스토리지 생성시 크기는 `사용자 가이드 > Compute > Instance > 콘솔 사용 가이드 > 인스턴스 생성 > 블록 스토리지 크기` 를 참고 |
