...
> [주의] 인스턴스의 루트 볼륨은 타입 변경으로 바꿀 수 없습니다.

### 루트 볼륨 설정

루트 볼륨은 인스턴스의 부팅에 사용하는 볼륨입니다. 인스턴스의 루트 볼륨으로 사용할 블록 스토리지 타입과 크기를 결정합니다.

- **HDD** 또는 **SSD** 중 하나를 선택합니다. 타입에 따라 요금과 성능이 달라집니다.
- 한번 선택한 블록 스토리지 타입은 변경할 수 없습니다.

인스턴스의 루트 볼륨 크기는 인스턴스 타입에 따라 달라집니다.
...
- 인스턴스가 한번 생성되면 루트 볼륨으로 사용한 블록 스토리지 크기는 변경할 수 없습니다. 인스턴스의 용량이 부족하다면 블록 스토리지를 추가하여 사용해야 합니다.
- 블록 스토리지 크기는 이미지가 요구하는 최소 크기 이상으로 만들어야 합니다.

>[참고] 블록 스토리지 크기에 따라 과금되므로 루트 볼륨 크기를 무조건 크게 만드는 것은 비효율적입니다. 필요에 따라 블록 스토리지를 추가하여 사용하는 것이 좋습니다.
...
인스턴스 생성 후 추가 볼륨 여부를 지정합니다. 추가 블록 스토리지 사용을 선택하면, 루트 볼륨에 사용 중인 블록 스토리지와 별개로 새로운 블록 스토리지를 생성하여 인스턴스에 연결합니다. 루트 볼륨과 마찬가지로 추가 볼륨을 생성할 때 이름, 스토리지 타입, 크기를 지정할 수 있습니다.

루트 볼륨은 OS 용도로만 사용하고 추가 볼륨에 자주 사용하는 응용 프로그램이나 데이터를 보관하면 블록 스토리지 연결/해제 또는 스냅샷 기능을 통해 쉽게 이전하거나 복제할 수 있습니다. 또한 인스턴스 장애가 발생했을 때, 블록 스토리지 연결만 해제한 뒤 다른 인스턴스에 연결하여 쉽게 서비스를 복구할 수 있습니다.
...
인스턴스의 루트 볼륨으로부터 이미지를 생성합니다. 이미지 생성은 데이터 정합성을 보장하기 위해 인스턴스를 중지한 상태에서 진행하는 것을 권장합니다.

인스턴스의 루트 볼륨 여유 공간이 전혀 없는 상태에서 이미지 생성은 가능하나, 이미지를 다른 인스턴스에서 사용하기 위한 초기화 작업을 할 수 없어서 정상 사용이 불가능합니다. 이미지를 생성하기 전에 인스턴스에서 최소 100KB의 여유 공간을 확보합니다.
...
> [참고] 인스턴스의 루트 볼륨 크기는 변경할 수 없습니다. 인스턴스의 블록 스토리지 공간이 부족하다면 블록 스토리지를 추가하여 사용합니다. 자세한 블록 스토리지 추가 방법은 [블록 스토리지 개요](/Storage/Block%20Storage/ko/overview/)를 참고합니다.
