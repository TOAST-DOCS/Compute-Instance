...
인스턴스 키페어는 인스턴스가 활성 상태인 경우에만 변경할 수 있습니다. 인스턴스 기본 계정의 키페어를 선택한 키페어로 변경합니다. 인스턴스 기본 계정은 인스턴스 상세 화면의 **접속 정보** 탭에서 확인할 수 있습니다.

> [주의] 인스턴스 키페어 변경 시 선택한 키페어를 제외한 인스턴스 내 모든 공개 키 내용이 삭제됩니다.

> [참고] <br>인스턴스 키페어 변경은 기본 인프라 서비스 ADMIN 권한이 있는 프로젝트 멤버만 사용 가능하며, Windows OS 인스턴스인 경우 사용이 불가능합니다. 일부 이미지에서는 인스턴스 키페어 변경을 지원하지 않을 수 있습니다.
