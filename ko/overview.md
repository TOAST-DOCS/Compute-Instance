...
인스턴스는 가상의 CPU, 메모리, 기본 디스크로 구성된 가상의 서버입니다. 이 서버에 고객의 서비스나 애플리케이션을 설치하고 TOAST가 제공하는 다양한 서비스를 조합하여 사용합니다.
...
- **키페어**(Key-pair): 인스턴스 접속 수단
- **보안그룹**(Security group): 인스턴스 네트워크 보안 설정,
- **네트워크**: 인스턴스가 연결될 가상 네트워크

이 정보에 따라 인스턴스의 속성과 사용 방식이 바뀝니다. 이 정보 가운데 이미지, 가용성 영역을 제외한 설정은 인스턴스 생성 이후에도 변경 가능합니다. 일부 인스턴스 사양은 인스턴스 생성 이후에 변경할 수 없습니다. 인스턴스 사양 변경에 대한 자세한 정보는 [Compute > Instance > 콘솔 사용 가이드](./console-guilde/)를 참조합니다.
...
모든 이미지는 인스턴스의 가상 하드웨어에서 최적으로 실행되도록 설정되어 있으며, TOAST의 보안 검증을 거쳤기 때문에 안전하게 사용 가능합니다. 이미지에 대한 자세한 설명은 [Compute > Image](service/compute/image)를 참조합니다.
...
가장 저렴한 인스턴스입니다. 워크로드가 높지 않은 서버에서 보통 사용합니다. 로컬 디스크를 사용하기 때문에 다른 인스턴스보다 안정성이 상대적으로 떨어지지만 저렴한 가격에 이용할 수 있습니다. 이 타입의 인스턴스는 I/O 성능을 보장하지 않습니다.
...
키페어는 공개 키 기반 구조([PKI](https://ko.wikipedia.org/wiki/%EA%B3%B5%EA%B0%9C_%ED%82%A4_%EA%B8%B0%EB%B0%98_%EA%B5%AC%EC%A1%B0)) 바탕의 SSH 공개키, 개인키 쌍입니다. TOAST에서 생성한 인스턴스에 접속하기 위해서는 보안에 취약한 키보드 입력 방식의 아이디/패스워드 인증 방식 대신 키페어를 이용해야 합니다. 사용자는 키페어의 개인키를 이용하여 로그인 정보를 안전하게 인스턴스로 전송하고 인스턴스로 접속하게 됩니다. 키페어를 이용해서 인스턴스에 접속하는 방법은 [인스턴스 접속 방법](./overview)를 참조합니다.

키페어는 인스턴스를 생성할 때 만들 수도 있고, 기존에 만들어 둔 키페어를 사용할 수도 있습니다. 키페어 등록 방법은 [콘솔 가이드](./console-guide)의 **키페어 가져오기**를 참조합니다.

> [주의]
키페어를 새로 생성하게 되면 키페어의 개인키를 다운로드하게 됩니다. 개인키는 두 번 발급되지 않으니, 다운로드한 개인키는 안전한 디스크나 USB 드라이브 등에 잘 보관하시기 바랍니다. 개인키는 외부에 유출되면 누구나 유출된 개인키를 통해 해당 인스턴스에 접근할 수 있게 되므로 신중하게 관리해야 합니다.

### 보안 그룹

보안 그룹은 인스턴스에 전달되는 네트워크 트래픽을 결정하는 가상의 방화벽입니다. 보안 그룹에 대한 자세한 설명은 [Network > VPC](service/network/vpc)를 참조합니다.
...
인스턴스가 외부와 통신을 하려면 VPC에서 정의된 네트워크 중 적어도 하나 이상에 연결되어 있어야 합니다. 네트워크에 연결되어 있지 않는 인스턴스는 접근할 수 없습니다. 네트워크를 새로 생성하거나 변경하려면 [Network > VPC](service/network/vpc)를 참조합니다.


## 과금

인스턴스는 생성하는 순간부터 과금됩니다. 인스턴스의 기본 디스크는 인스턴스 생성과 별도로 블록 스토리지의 과금 체계와 동일하게 과금됩니다.

인스턴스는 상태에 관계없이 과금됩니다. 인스턴스가 실행 중이거나 종료되어 있는 상태에서도 과금됩니다. 필요없는 인스턴스는 삭제하여야 과금이 되지 않습니다.
...
리눅스 인스턴스에 접속할 때는 ssh 클라이언트를 이용합니다. 인스턴스의 보안 그룹에 ssh 접근 포트(기본값 22)가 열려 있지 않다면 접속이 불가능합니다. ssh 접근을 허용하는 방법에 대해서는 [Network > VPC](service/network/vpc)를 참조하세요. 인스턴스에 플로팅 IP가 할당되어 있지 않다면 TOAST 외부에서 접속할 수 없습니다. 플로팅 IP를 할당하는 방법에 대해서는 [Network > VPC](service/network/vpc)를 참조하세요.
...
**Ubuntu / Debian**의 기본 사용자 ID는 `ubuntu`이며,
...
* 키페어의 개인키를 PuTTY용 개인키로 변경
* PuTTY용 개인키를 PuTTY에 등록
* PuTTY로 인스턴스에 접속

PuTTY에서는 키페어 개인키를 PuTTY의 개인키 형식으로 바꾸어서 사용해야 합니다. PuTTY를 설치하면 같이 설치되는 puttygen를 실행합니다.
...
> [주의]
인스턴스에 자동으로 로그인하여야 하는 경우에는 암호어구를 사용하지 않아야 합니다. 암호어구를 사용하면 로그인을 할 때 개인키에 대한 패스워드를 키보드로 직접 입력해야 합니다.
...
윈도우 서버에 접속하려면, TOAST 콘솔의 Compute > Instance > Management 페이지에서 접속하려는 윈도우 인스턴스를 선택합니다. 하단 상세화면의 **인스턴스 접속** 탭에서 **Password 확인** 버튼을 클릭하여 윈도우 서버에 설정된 패스워드를 확인합니다.

![윈도우 인스턴스 접속](http://static.toastoven.net/prod_instance/windows-login.png)

패스워드 확인 기능에 입력하는 키페어의 개인키는 서버로 전송되지 않고, 브라우저에서 패스워드를 복호화하는 작업에만 사용됩니다.

**패스워드 확인** 기능 옆의 **연결** 버튼으로 원격 데스크톱 접속 설정이 담겨 있는 rdp 파일을 받아서 실행하면, 윈도우 서버에 접속합니다. 윈도우 서버의 ID는 `Administrator`이며, 패스워드는 TOAST 콘솔에서 방금 확인한 패스워드를 이용합니다.
