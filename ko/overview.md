...
인스턴스는 가상의 CPU, 메모리, 기본 디스크로 구성된 가상 서버입니다. 이 서버에 고객의 서비스나 애플리케이션을 설치하고 NHN Cloud가 제공하는 다양한 서비스를 조합하여 사용합니다.
...
이미지는 운영체제를 담고 있는 가상 디스크입니다. NHN Cloud는 현재 CentOS, Debian, Ubuntu와 Windows를 지원하고 있습니다. 자세한 지원 운영체제 버전은 [엔에이치엔 클라우드(NHN Cloud) 서비스 소개](https://toast.com/service/compute/instance)를 참고합니다.
...
* 인스턴스 기본 블록 스토리지는 인스턴스와 별도로 블록 스토리지 과금 기준으로 과금됩니다.
* 중지된 인스턴스에 대해서는 인스턴스의 블록 스토리지만 과금됩니다.
