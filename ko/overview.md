...
인스턴스는 가상의 서버입니다. 용도에 맞는 서버 사양(flavor)과 운영체제를 선택하고, 인스턴스를 생성하기만 하면 즉시 접속하여 사용할 수 있는 가상 서버가 수 분 안에 준비됩니다.

인스턴스는 가상의 CPU, 메모리, 기본 디스크로 구성됩니다. CPU와 메모리는 인스턴스의 사양(flavor)에 따라 정해지며, 기본 디스크는 TOAST에서 제공하는 이미지에서 만들어집니다. 이 이미지에 설치되어 있는 운영체제에 따라 인스턴스의 운영체제가 결정됩니다.

인스턴스를 만들 때에는, 

- **이미지**로 인스턴스의 운영체제를,
- **사양**으로 인스턴스의 가상 하드웨어 성능을,
- **가용성 영역**(AZ, Availability zone)으로 인스턴스가 만들어질 물리적인 위치를,
- **키페어**(Key-pair)로 인스턴스에 접속 수단을,
- **보안그룹**(Security group)으로 인스턴스의 네트워크 보안 설정을,
- **네트워크**로 인스턴스가 연결될 가상 네트워크를

결정하게 됩니다. 이 정보에 따라 인스턴스의 속성과 사용 방식이 바뀝니다. 이 정보 가운데 이미지와 가용성 영역을 제외한 설정은 인스턴스 생성 이후에도 변경 가능합니다.

### 이미지

이미지는 운영체제를 담고 있는 가상 디스크입니다. TOAST는 현재 CentOS, Debian, Ubuntu 배포판이 설치된 리눅스 이미지와 윈도우 2008, 2012, 2016 이 설치된 윈도우 이미지를 제공합니다. 모든 이미지는 인스턴스의 가상 하드웨어에서 최적으로 실행되도록 설정되어 있으며, TOAST의 보안 검증을 거쳐 안전하게 사용 가능합니다. 이미지에 대한 자세한 설명은 [Compute > Image](service/compute/image)를 참조합니다.
...
TOAST에서 생성한 인스턴스에 접속하기 위해서는 보안에 취약한 키보드 입력 방식의 패스워드 인증 방식 대신 키페어를 이용해야 합니다. 키페어는 인스턴스를 생성할 때 만들 수도 있고, 기존에 만들어둔 키페어를 사용할 수도 있습니다. 키페어 등록 방법은 콘솔 가이드를 참조합니다.
