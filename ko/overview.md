...
인스턴스는 가상의 CPU, 메모리, 블록 스토리지로 구성된 가상 서버입니다. 이 서버에 고객의 서비스나 애플리케이션을 설치하고 NHN Cloud가 제공하는 다양한 서비스를 조합하여 사용합니다.
...
| u2 | 가장 저렴한 인스턴스입니다. 워크로드가 높지 않은 서버에서 사용합니다.<br>로컬 디스크를 사용하기 때문에 상대적으로 다른 인스턴스보다 안정성이 떨어지지만 저렴한 가격에 이용할 수 있습니다.<br>이 타입의 인스턴스는 I/O 성능을 보장하지 않습니다. |
| x1 | 고사양의 CPU와 메모리를 지원하는 타입입니다. 높은 성능이 필요한 서비스나 애플리케이션에 사용합니다. |

### 볼륨

인스턴스에 연결하여 사용하는 스토리지 공간입니다. 인스턴스의 부팅에 사용하는 루트 볼륨을 필수로 구성하고, 그 외 추가 볼륨을 사용자의 필요에 맞게 추가할 수 있습니다. 인스턴스 볼륨으로 블록 스토리지를 연결할 수 있으며, u2 타입은 로컬 디스크를 루트 볼륨으로 사용합니다.

인스턴스를 생성하면 부팅에 사용하기 위해 선택한 이미지가 루트 볼륨에 저장됩니다. NHN Cloud는 현재 CentOS, Debian, Ubuntu와 Windows를 지원하고 있습니다. 자세한 지원 운영체제 버전은 [엔에이치엔 클라우드(NHN Cloud) 서비스 소개](https://toast.com/service/compute/instance)를 참고합니다.
...
NHN Cloud는 물리 하드웨어 문제로 생기는 장애에 대비하기 위해 전체 시스템을 여러 개의 가용성 영역으로 나누어 운영합니다. 이 가용성 영역별로 저장 시스템, 네트워크 스위치, 상면, 전원 장치가 모두 별도로 구성돼 있습니다. 한 가용성 영역 내에서 생기는 장애는 다른 가용성 영역에 영향을 주지 않으므로 서비스 전체의 가용성이 높아집니다. 인스턴스를 여러 가용성 영역에 나눠 구축한다면 서비스의 가용성을 더욱 높일 수 있습니다.
...
* 인스턴스 루트 볼륨은 인스턴스와 별도로 블록 스토리지 과금 기준으로 과금됩니다.
* 중지된 인스턴스에 대해서는 인스턴스의 루트 볼륨과 (추가한 경우) 추가 볼륨만 과금됩니다.
