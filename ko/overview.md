...
부팅 실패, 네트워크 구성 문제와 같이 SSH 클라이언트를 사용할 수 없는 상황에서 직렬 콘솔에 연결하여 인스턴스에 접속할 수 있습니다. 

직렬 콘솔 기능은 다음과 같은 제약 조건이 있습니다.

* 인스턴스당 하나의 직렬 콘솔 연결만 가능하며, 다중 접속 시도 시 정상적으로 연결이 되지 않을 수 있습니다.
* 개인이 업로드한 이미지로 만든 인스턴스와 개인 이미지로 만든 인스턴스는 직렬 콘솔 접속을 보장하지 않습니다.
* 직렬 콘솔 연결은 최대 10분간 접속 가능합니다.
* Windows 인스턴스는 직렬 콘솔 기능을 지원하지 않습니다.

> [주의]
> 일반적으로 SSH 클라이언트 접속을 권장하며 직렬 콘솔에 접속하여 부팅 방법 변경 시 부팅을 실패할 수 있으며, 이에 따른 결과에 대한 책임은 사용자에게 있습니다.

#### GRUB 부트로더 설정 변경

2024. 11. 26. 배포 이전에 생성한 인스턴스에서 부트로더를 조작하기 위해서는 GRUB 설정이 필요합니다.
